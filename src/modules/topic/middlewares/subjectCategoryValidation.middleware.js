...
export const validatePagination = (req, res, next) => {
  ...
  if (page !== undefined && page !== null) {
    ...
    if (!Number.isInteger(pageNum) || pageNum <= 0) {
      return res.status(400).json({
        success: false,
        message: "Trang (page) phải l�  số nguyên dương",
        code: "INVALID_PAGE",
        data: null
      });
    }
  ...
  if (limit !== undefined && limit !== null) {
    ...
    if (!Number.isInteger(limitNum) || limitNum <= 0) {
      return res.status(400).json({
        success: false,
        message: "Số lượng bản ghi mỗi trang (limit) phải l�  số nguyên dương",
        code: "INVALID_LIMIT",
        data: null
      });
    }
...
export const validateCreateSubjectCategory = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(areaIdNum) || areaIdNum <= 0) {
      return res.status(400).json({
        success: false,
        message: "Mã lĩnh vực subject_area_id phải l�  số nguyên dương hợp lệ",
        code: "INVALID_SUBJECT_AREA_ID",
        data: null
      });
    }

    // Kiểm tra sự tồn tại v�  hoạt động của Subject Area
    ...
    if (display_name === undefined || display_name === null || typeof display_name !== "string" || display_name.trim() === "") {
      return res.status(400).json({
        success: false,
        message: "Tên hiển thị display_name không được để trống v�  phải l�  một chuỗi ký tự",
    ...
    if (duplicateName) {
      return res.status(400).json({
        success: false,
        message: "Tên hiển thị display_name đã tồn tại trong lĩnh vực n� y",
