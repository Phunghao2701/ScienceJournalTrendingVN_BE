...
const _getTopJournals = async ({ years = 2, limit = 10 } = {}) => {
  ...
  const windowResult = await prisma.$queryRawUnsafe(`
  ...
  const toYear = windowResult[0]?.to_year;
  if (!toYear) {
    return {
      window: {
        from_year: null,
  ...
  const keywordsByJournal = groupRowsByJournal(keywordsResult);
  const topicsByJournal = groupRowsByJournal(topicsResult);
...
const _getTopUniversities = async ({ years = 2, limit = 10, hot_limit = 10 } = {}) => {
  ...
  const hotKeywordIds = hotKeywordsResult.map((row) => row.keyword_id);
  const hotTopicIds = hotTopicsResult.map((row) => row.topic_id);
  ...
  const universitiesResult = await prisma.$queryRawUnsafe(
    `
    WITH recent_articles AS (
      SELECT
        a."article_id",
  ...
  const universities = universitiesResult;
  const institutionKeys = universities.map((row) => row.institution_key);

  if (institutionKeys.length === 0) {
    return {
      window: { from_year: fromYear, to_year: toYear, years: yearsWindow },
      hot_basis: { keywords: hotKeywordsResult, topics: hotTopicsResult },
      items: [],
    };
  }
  ...
  const keywordsByInstitution = groupRowsByInstitution(keywordsResult, "institution_key");
  const topicsByInstitution = groupRowsByInstitution(topicsResult, "institution_key");
  const articlesByInstitution = groupRowsByInstitution(articlesResult, "institution_key");

  return {
    window: { from_year: fromYear, to_year: toYear, years: yearsWindow },
    hot_basis: {
      keywords: hotKeywordsResult,
      topics: hotTopicsResult,
    },
...
const _getJournalRankings = async ({ limit = 10 } = {}) => {
  ...
  return {
    items: result.map((row, index) => ({ rank: index + 1, ...row })),
  };
};
...
const _getTrendingArticles = async ({ years = 2, limit = 10, hot_limit = 10 } = {}) => {
  ...
  const relationTablesResult = await prisma.$queryRawUnsafe(
    `
    SELECT table_name
  ...
  const relationTables = new Set(relationTablesResult.map((row) => row.table_name));
  ...
  const articlesResult = await prisma.$queryRawUnsafe(
    `
    WITH recent_articles AS (
      SELECT
        a."article_id",
  ...
  if (articleIds.length === 0) {
    return { window: { from_year: fromYear, to_year: toYear, years: yearsWindow }, hot_basis: { keywords: hotKeywordsResult, topics: hotTopicsResult }, items: [] };
  }
  ...
  const keywordsByArticle = groupByArticle(keywordsResult);
  const topicsByArticle = groupByArticle(topicsResult);
  const authorsByArticle = groupByArticle(authorsResult);
...
const _getTrendingKeywords = async ({ limit = 10, hot_limit = 10 } = {}) => {
  ...
  const hotTopicsResult = await prisma.$queryRawUnsafe(
    `
    WITH article_topics AS (
      SELECT DISTINCT a."article_id", a."primary_topic" AS "topic_id"
      FROM "Article" a
  ...
  const hotTopicIds = hotTopicsResult.map((topic) => topic.topic_id);
  ...
  return {
    hot_basis: { topics: hotTopicsResult },
    items: keywordsResult.map((row, index) => ({ rank: index + 1, ...row })),
  };
};
