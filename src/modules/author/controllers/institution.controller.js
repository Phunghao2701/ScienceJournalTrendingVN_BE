...
export const getInstitutionById = async (request, reply) => {
  try {
    ...
    return reply.status(200).send({ success: true, message: "Lấy chi tiết cơ sở nghiên cứu thành công", data: institution });
  } catch (error) {
...
export const getInstitutions = async (request, reply) => {
  try {
    ...
    return reply.status(200).send({ success: true, message: "Lấy danh sách cơ sở giáo dục thành công", data: result.data, pagination: result.pagination });
  } catch (error) {
