...
export const resolveAnalysisWindows = async (params = {}) => {
  ...
  const toYear = rows[0]?.to_year ? Number(rows[0].to_year) : null;
  if (!toYear) {
    return {
...
export const getArticleAnalysis = async (params = {}) => {
  ...
  const [
    ...
    prisma.$queryRawUnsafe(summaryQuery, ...preparedWindow.values),
    prisma.$queryRawUnsafe(worksQuery, ...preparedWindow.values),
    prisma.$queryRawUnsafe(citationsQuery, ...preparedWindow.values),
    prisma.$queryRawUnsafe(topEntityQueries.institutions.sql, ...topEntityQueries.institutions.values),
    prisma.$queryRawUnsafe(topEntityQueries.authors.sql, ...topEntityQueries.authors.values),
    prisma.$queryRawUnsafe(topEntityQueries.journals.sql, ...topEntityQueries.journals.values),
    prisma.$queryRawUnsafe(topEntityQueries.topics.sql, ...topEntityQueries.topics.values),
    prisma.$queryRawUnsafe(topEntityQueries.keywords.sql, ...topEntityQueries.keywords.values),
    prisma.$queryRawUnsafe(growthEntityQueries.institutions.sql, ...growthEntityQueries.institutions.values),
    prisma.$queryRawUnsafe(growthEntityQueries.authors.sql, ...growthEntityQueries.authors.values),
    prisma.$queryRawUnsafe(growthEntityQueries.journals.sql, ...growthEntityQueries.journals.values),
    prisma.$queryRawUnsafe(growthEntityQueries.topics.sql, ...growthEntityQueries.topics.values),
    prisma.$queryRawUnsafe(growthEntityQueries.keywords.sql, ...growthEntityQueries.keywords.values),
    prisma.$queryRawUnsafe(trendingQuery, ...trendingPrepared.values),
    prisma.$queryRawUnsafe(coverageQuery, ...preparedWindow.values),
  ]);

  const topEntityResults = {
    institutions: mapGrowthRows(topInstitutionsRows),
    authors: mapGrowthRows(topAuthorsRows),
    journals: mapGrowthRows(topJournalsRows),
    topics: mapGrowthRows(topTopicsRows),
    keywords: mapGrowthRows(topKeywordsRows),
  };
  const growthEntityResults = {
    institutions: mapGrowthRows(growthInstitutionsRows),
    authors: mapGrowthRows(growthAuthorsRows),
    journals: mapGrowthRows(growthJournalsRows),
    topics: mapGrowthRows(growthTopicsRows),
    keywords: mapGrowthRows(growthKeywordsRows),
  };
  const totalArticlesWithHistory = citationsRows
  ...
  return {
    scope: filter.scope,
    window,
    summary: summaryRows[0] || emptySeries.summary,
    works_over_time: zeroFill(window.years, worksRows, (year, row) => ({
      year,
      count: row?.count || 0,
    })),
    citations_over_time: zeroFill(window.years, citationsRows, (year, row) => ({
      year,
    ...
    trending_articles: trendingRows,
    trending_article_coverage: coverageRows[0] || { eligible_articles: 0, total_articles: 0 },
  };
};
