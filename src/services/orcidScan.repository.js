    ...
    if (existing.is_deleted) {
      if (isTarget) {
        const error = new Error(
          "Tác giả mang ORCID này đang ở trạng thái đã xóa",
        );
...
const persistArticleBundle = async (
  ...
  if (!row) {
    const error = new Error(
      "Không thể tạo hoặc cập nhật bài báo do xung đột đồng thời",
    );
...
const persistResolvedChunk = async (client, items, dimensions) => {
  ...
  if (result.rows.length !== items.length) {
    const error = new Error(
      "Không thể tạo hoặc cập nhật đầy đủ chunk bài báo",
    );
