...
import { fetchOpenAlexWorksByIds } from "./openAlexApi.service.js";
import {
  normalizeDoi,
  normalizeOpenAlexId,
} from "../utils/orcid.js";
...
const loadHydrationState = async (databasePool, articleId) => {
  const articleResult = await databasePool.query(
    `
...
const persistReferences = async (
  ...
  try {
    ...
    const existingKeys = new Set(
      existingResult.rows.map((row) => row.reference_key),
    );
    ...
    await client.query({
      ...
      `,
      values: [articleId, JSON.stringify(records)],
    });
...
export const hydrateArticleReferences = async (
  ...
  if (
    ...
    throw createHydrationError(
      "OpenAlex không khả dụng và không có reference cục bộ",
      502,
