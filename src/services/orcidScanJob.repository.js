...
export const createOrReuseOrcidScanJob = async (
  ...
  const existing = await databasePool.query(activeJobQuery, [
    ACTIVE_STATUSES,
    requestedBy,
    orcid,
  ]);
  const resolved = resolveExistingJob(existing.rows[0], requestedBy, orcid);
  ...
  try {
    ...
    const raced = await databasePool.query(activeJobQuery, [
      ACTIVE_STATUSES,
      requestedBy,
      orcid,
    ]);
...
export const getOrcidScanJobPublications = async (
  ...
  const totalAvailable = Number(result.rows[0]?.total_available || 0);
  const rows = result.rows.filter((row) => row.item_id != null);
